...
type FetchDataInCollectionResult struct {
	...
	// 不存在的主键列表
	NotFoundIDs []interface{} `json:"ids_not_exists,omitempty"`
}

// PreviewDataRequest 表示预览数据的请求
type PreviewDataRequest struct {
	// 限制返回的数据条数
	Limit *int64 `json:"Limit,omitempty"`
	
	// 是否返回下载URL
	ReturnDownloadURL bool `json:"ReturnDownloadURL,omitempty"`
}

// PreviewDataResponse 表示预览数据的响应
